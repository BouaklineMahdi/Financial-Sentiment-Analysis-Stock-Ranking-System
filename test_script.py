...
def rank_companies(model, features, sp500_symbols,amount):
    try:
        # Predictions
        ...
        print("Length of predictions_flat:", len(predictions_flat))
        
        # Ensure arrays have the same length
        min_length = min(len(sp500_symbols), len(predictions_flat))
        sp500_symbols = sp500_symbols[:min_length]
        predictions_flat = predictions_flat[:min_length]
        
        # Create DataFrame
        df_ranked_companies = pd.DataFrame({"Symbol": sp500_symbols, "Prediction": predictions_flat})
        
        # Sort DataFrame by Prediction and select top 5
        top_5_companies = df_ranked_companies.sort_values(by="Prediction", ascending=False).head(amount)
...
def fetch_stock_data(symbol):
    try:
        with ThreadPoolExecutor(max_workers=3) as executor:
            # Download historical data, fetch financial data, and fetch additional info concurrently
            historical_data_future = executor.submit(download_historical_data, symbol)
            financial_data_future = executor.submit(fetch_financial_data, symbol)
            additional_info_future = executor.submit(fetch_additional_info, symbol)

            # Wait for all futures to complete
            historical_data = historical_data_future.result()
            financial_data = financial_data_future.result()
            additional_info = additional_info_future.result()
...
# Example usage:
# historical_data, financial_data, additional_info = fetch_stock_data('AAPL')
...
def main():
    ...
    for symbol in sp500_symbols:
        ...
        features_i, targets_i = preprocess_data(historical_data)
    ...
    features = [pd.DataFrame(arr) for arr in features]
    targets = [pd.DataFrame(arr) for arr in targets]
    ...
    try:
        while (amount>=1):
            ranked_companies = rank_companies(model, features, sp500_symbols, round(amount))
            model = train_model(features, targets)
